        ...
        while True:
            ...
            # --- GOTOWOŚĆ GRACZA (ZATWIERDZENIE PLANSZY) ---
                ...
                if current_room_code and current_room_code in rooms:
                    ...
                    # Sprawdzamy czy obaj są gotowi
                    p1 = room["players"][0]
                    p2 = room["players"][1]
                    
                    if p1 and p1.get("ready") and p2 and p2.get("ready"):
                        logging.info(f"Pokój {current_room_code}: Obaj gracze gotowi. Start bitwy!")
                        battle_msg = json.dumps({"status": "battle_start"})
                        try:
                            p1["conn"].sendall(str.encode(battle_msg))
                            p2["conn"].sendall(str.encode(battle_msg))
                        except Exception as e:
