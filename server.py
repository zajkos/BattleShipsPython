...
try:
    ...
    logging.info("Pula połączeń PostgreSQL (Threaded) zainicjalizowana.")
...
def validate_board(board):
    """Sprawdza autorytatywnie poprawność układu statków przesłanego przez klienta."""
    if not isinstance(board, list) or len(board) != 9:
        return False
    
    expected_lengths = [4, 3, 3, 2, 2, 1, 1, 1, 1]
    actual_lengths = []
    all_cells = set()
    ...
    for ship in board:
        ...
        horizontal = ship.get("horizontal")
        grid_pos = ship.get("grid_pos")
        cells = ship.get("cells")
        
        # 1. Walidacja typów danych
        if not isinstance(length, int) or length not in [1, 2, 3, 4]:
            return False
        if not isinstance(horizontal, bool):
            return False
        if not isinstance(grid_pos, list) or len(grid_pos) != 2:
            return False
        ...
        # 2. Walidacja współrzędnych i ułożenia
        col, row = grid_pos[0], grid_pos[1]
        ...
        for i, cell in enumerate(cells):
            ...
            # Sprawdzenie czy komórki odpowiadają wybranej pozycji i kierunkowi
            if horizontal:
                if c != col + i or r != row:
                    return False
            else:
            ...
            # Sprawdzenie sąsiedztwa i kolizji (odstęp 1 kratka od innych statków)
            for dc in range(-1, 2):
                for dr in range(-1, 2):
                    if (c + dc, r + dr) in all_cells:
                        return False
                        
        # Dodanie zatwierdzonych komórek do zbioru
        for cell in cells:
            all_cells.add((cell[0], cell[1]))
            
    # Sprawdzenie czy zestaw długości statków jest dokładnie taki, jak oczekiwano
    if sorted(actual_lengths) != sorted(expected_lengths):
        return False
        
    return True

def check_password_strength(password):
    """Sprawdza autorytatywnie wymogi siły hasła (KAN-72)."""
    if len(password) < 8:
        return False
    ...
    try:
        while True:
            ...
            if action == "register":
                ...
                if not u or not p or not check_password_strength(p):
                    send_msg(conn, {"status": "error", "message": "Niepoprawne dane (hasło za słabe)."})
                    continue
                ...
                try:
                    with db_conn.cursor() as cur:
                        ...
                        if cur.fetchone():
                            ...
                            # Obliczamy hasz SHA-256 hasła przysłanego przez klienta
                            client_hash = hashlib.sha256(p.encode('utf-8')).hexdigest()
                            hashed = bcrypt.hashpw(client_hash.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
                ...
                if username is not None:
                    send_msg(conn, {"status": "error", "message": "Jesteś już zalogowany na tej sesji!"})
                    continue
                db_conn = db_pool.getconn()
                try:
                    with db_conn.cursor() as cur:
                        ...
                        if row and p:
                            client_hash = hashlib.sha256(p.encode('utf-8')).hexdigest()
                            if bcrypt.checkpw(client_hash.encode('utf-8'), row[1].encode('utf-8')):
                                uid = row[0]
                                with auth_lock:
                ...
                if not validate_board(board):
                    send_msg(conn, {"status": "error", "message": "Niepoprawny układ planszy."})
                    continue
                ...
                if not isinstance(x, int) or not isinstance(y, int) or not (0 <= x < 10 and 0 <= y < 10):
                    continue
                if not current_room_code:
                    current_room_code, room = find_room_by_conn(conn)
                else:
    ...
    while True:
        ...
        try:
            ...
            with rooms_lock:
                ...
                for code, room in list(rooms.items()):
                    # Sprawdzenie czy gra się toczy (2 graczy i obaj gotowi, gra się nie skończyła)
                    if len(room["players"]) == 2 and all(p and p["ready"] for p in room["players"]) and not room.get("game_over"):
            ...
            logging.error(f"Błąd w wątku turn_timer_checker: {e}")
...
def start_server():
    ...
    if USE_SSL:
        try:
            ...
            logging.info(f"Serwer SSL/TLS uruchomiony na porcie {PORT}")
        except Exception as e:
            logging.error(f"Błąd SSL (brak cert.pem/key.pem). Serwer działa bez szyfrowania! Błąd: {e}")
    ...
    while True:
        try:
            ...
            logging.error(f"Błąd akceptacji połączenia: {e}")
