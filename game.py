...
def play_game(screen, p1_name, p2_name, net):
    ...
    # Inicjalizacja floty (1x4, 2x3, 3x2, 4x1)
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if len(placed_ships) == 10 and btn_ready.handle_event(event):
            ...
            try:
                ...
                if data:
                    ...
                    if response.get("status") == "battle_start":
                        if net: net.client.setblocking(True)
                        print("Faza bitwy uruchomiona! Przejście do kolejnego ekranu (TODO).")
                        return "BATTLE_PHASE"
                    elif response.get("status") == "opponent_disconnected":
