...
def battle_phase(screen, p1_name, p2_name, net, player_idx, initial_turn, my_fleet):
    ...
    # Ustawienia plansz (lewa dla gracza, prawa dla wroga)
    grid_size = 600
    ...
    # Historie strzałów: zbiory krotek (x, y)
    my_shots_hit = set()
    my_shots_miss = set()
    enemy_shots_hit = set()
    enemy_shots_miss = set()

    # Punkty początkowe (zostaną zaktualizowane, gdy serwer prześle pełną logikę zatapiania)
    my_score = 0
    enemy_score = 0
    ...
    while True:
        ...
        # Tworzymy tymczasową powierzchnię do rysowania wszystkiego (aby łatwo przesunąć całość)
        display_surface = pygame.Surface((WIDTH, HEIGHT))
        display_surface.fill(BG_COLOR)
        
        mouse_pos = pygame.mouse.get_pos()
        # Korekcja pozycji myszy względem offsetu (opcjonalne, ale lepiej rysować UI bez przesunięcia)
        # Tu przesuniemy TYLKO elementy gry, UI zostanie statyczne dla czytelności
        
        # UI - Kto teraz strzela
        ...
        display_surface.blit(enemy_label, enemy_label.get_rect(center=(enemy_grid_x + grid_size // 2, enemy_grid_y - 80)))

        # Tabela punktów (Zgodna z KAN - Tabela wyników na ekranie walki)
        my_score_surf = font_score.render(f"Punkty: {my_score}", True, (255, 215, 0))
        display_surface.blit(my_score_surf, my_score_surf.get_rect(center=(my_grid_x + grid_size // 2, my_grid_y + grid_size + 40)))

        enemy_score_surf = font_score.render(f"Punkty: {enemy_score}", True, (255, 215, 0))
        display_surface.blit(enemy_score_surf,
                    enemy_score_surf.get_rect(center=(enemy_grid_x + grid_size // 2, enemy_grid_y + grid_size + 40)))

        # Rysowanie plansz
        draw_grid(display_surface, my_grid_x, my_grid_y, grid_size)
        draw_grid(display_surface, enemy_grid_x, enemy_grid_y, grid_size)

        # -- NOWOŚĆ: Rysowanie własnej floty na lewej planszy --
        for ship in my_fleet:
            col, row = ship.grid_pos
            ship.cell_size = cell_size
            ship.update_to_grid_size(cell_size) # Upewniamy się, że rozmiar i obraz są poprawne dla siatki bitewnej
            ship.rect.x = my_grid_x + col * cell_size
            ship.rect.y = my_grid_y + row * cell_size
            ship.draw(display_surface)

        # Rysowanie strzałów na Tarczy Wroga (Prawa)
        for x, y in my_shots_miss:
            pygame.draw.circle(display_surface, (150, 150, 150), (enemy_grid_x + x * cell_size + cell_size // 2,
                                                         enemy_grid_y + y * cell_size + cell_size // 2), cell_size // 3)

        # Rysowanie strzałów na Naszej Flocie (Lewa)
        ...
        # Podświetlenie celownika (tylko w naszej turze)
        if is_my_turn:
        ...
        # Odbieranie wyników z serwera (fragment z animacjami pozostał bez zmian w logice)
        ...
        # Odbieranie wyników z serwera
        try:
            data = net.client.recv(2048).decode()
            if data:
                # Odbieramy wiele wiadomości, jeśli przyszły w jednym pakiecie
                ...
                for msg in messages:
                    ...
                    if response.get("status") == "shot_result":
                        ...
                        current_turn = response["next_turn"]  # Aktualizacja tury!

                        # --- NOWOŚĆ: Tworzenie animacji ---
                        if shooter == player_idx: # My strzelaliśmy (prawa plansza)
                            ax = enemy_grid_x + sx * cell_size + cell_size // 2
                            ay = enemy_grid_y + sy * cell_size + cell_size // 2
                        else: # Wróg strzelał (lewa plansza)
                        ...
                        # --- NOWOŚĆ: Stały dym przy trafieniu ---
                        if is_hit:
                            # Prędkość dymu może być nieco inna dla naturalnego wyglądu
                            persistent_effects.append(AnimationEffect(ax, ay, smoke_frames, speed=0.4, loop=True))

                        # --- NOWOŚĆ: Wyzwalanie Screen Shake ---
                        if is_hit:
                            shake_amount = 10 # Zmniejszono z 15
                            shake_timer = 15  # Skrócono czas trwania
                        elif not is_hit and shooter != player_idx:
                            # Opcjonalnie: lekki wstrząs gdy wróg pudłuje (fala uderzeniowa)
                            shake_amount = 3 # Zmniejszono z 5
                            shake_timer = 8

                        if shooter == player_idx:  # My strzelaliśmy
                            if is_hit:
                                my_shots_hit.add((sx, sy))
                            else:
                                my_shots_miss.add((sx, sy))
                        else:  # Przeciwnik strzelał
                            if is_hit:
                                enemy_shots_hit.add((sx, sy))
                            else:
                                enemy_shots_miss.add((sx, sy))

                        # --- NOWOŚĆ: Aktualizacja punktów ---
        ...
        # --- NOWOŚĆ: Aktualizacja i rysowanie stałych efektów (dym) ---
        for effect in persistent_effects:
            effect.update()
            effect.draw(display_surface)

        # --- NOWOŚĆ: Aktualizacja i rysowanie animacji ---
        ...
        # Na koniec nakładamy wszystko na główny ekran z przesunięciem (Shake)
