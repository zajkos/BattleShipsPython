...
def show_high_scores(screen, clock, net, background=None):
    ...
    # Przycisk powrotu
    btn_back = ImageButton(WIDTH // 2 - 130, HEIGHT - 100, "powrót.png", width=260)
    ...
    def update_table():
        ...
        for i, row in enumerate(scores_data):
            ...
            cx += cols[4]["width"]
            # Średnia
            table_surface.blit(font_row.render(f"{row['avg_points']:.2f}", True, (180, 180, 180)), (cx + 10, y + 5))
    ...
    title_surf = font_title.render("RANKING GRACZY", True, (212, 175, 55))
    ...
    header_surfaces = {}
    for col in cols:
        header_surfaces[(col["name"], True)] = font_header.render(col["name"], True, (212, 175, 55))
        header_surfaces[(col["name"], False)] = font_header.render(col["name"], True, (180, 180, 180))
        
    ind_up = font_header.render("▲", True, (212, 175, 55))
    ind_down = font_header.render("▼", True, (212, 175, 55))
    ...
    while True:
        ...
        if max_scroll > 0:
            pygame.draw.rect(screen, (40, 40, 50), sb_rect, border_radius=6)
            ...
            bar_color = BUTTON_HOVER_COLOR if (max_scroll > 0 and bar_rect.collidepoint(mouse_pos)) or dragging_scroll else (100, 100, 100)
