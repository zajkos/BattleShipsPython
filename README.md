...
| **Model Sieciowy** | Authoritative Client-Server with Optional SSL/TLS |
| **System Operacyjny** | Windows (Windows Server / Windows 10+) |
| **Główny Stack** | Python 3.10+, Pygame 2.x, PostgreSQL 14+ |
| **Format Komunikacji** | Length-Prefixed JSON over Sockets (Optional SSL/TLS) |
...
*   **Komunikacja Sieciowa**: Natywna biblioteka `socket` z opcjonalnym szyfrowaniem za pomocą modułu `ssl` (implementacja TLS 1.3, wyłączona domyślnie w opcji `USE_SSL = False`). Wykorzystanie protokołu **Length-Prefixing** (4-bajtowy nagłówek binarny) dla stabilnej obsługi fragmentacji TCP.
*   **Zarządzanie Danymi**:
    *   `psycopg2.pool`: Implementacja Connection Pooling (`ThreadedConnectionPool`) dla wysokiej wydajności przy wielu graczach.
...
*   **Credential Protection**: Hasła są haszowane przed zapisem przy użyciu `bcrypt` po uprzednim zmapowaniu do skrótu `SHA-256` (w formacie heksadecymalnym), co eliminuje ryzyko obcięcia haseł powyżej 72 znaków przez silnik `bcrypt`. Pliki `.env` są wykluczone z repozytorium.
...
#### Wizualizacja Przejść Stanów (State Flow)
```mermaid
stateDiagram-v2
    [*] --> AUTH : Uruchomienie gry
    AUTH --> LOBBY : Sukces logowania / rejestracji
    LOBBY --> MATCHMAKING : Szybka gra / Stworzenie pokoju / Dołączenie
    MATCHMAKING --> PLACEMENT : Znaleziono przeciwnika / Start sesji
    PLACEMENT --> BATTLE : Obaj gracze gotowi (player_ready)
    BATTLE --> ENDGAME : Wygrana / Przegrana / Poddanie / Walkower
    ENDGAME --> PLACEMENT : Rematch Handshake (Obaj chcą rewanżu)
    ENDGAME --> LOBBY : Powrót do menu głównego
    BATTLE --> LOBBY : Rozłączenie przeciwnika (walkower)
```
...
#### Diagram Przepływu Architektury (System Architecture Flow)
```mermaid
graph TD
    subgraph Klient ["Klient (Pygame)"]
        Main["main.py"] --> AuthScr["auth_screen.py"]
        Main --> GameMod["game.py"]
        GameMod --> ShipClass["ship.py"]
        GameMod --> NetClient["network.py"]
    end

    subgraph NetworkLayer ["Sieć (TCP Sockets)"]
        NetClient <-->|JSON| NetServer["server.py"]
    end

    subgraph ServerLayer ["Serwer"]
        NetServer --> Threads["Wątki (handle_client)"]
        NetServer --> Watchdog["Watchdog AFK"]
        Threads --> DB_Pool["psycopg2 pool"]
    end

    subgraph DBLayer ["Baza Danych"]
        DB_Pool <--> DB_Eng[("PostgreSQL")]
    end

    style NetServer fill:#1a233a,stroke:#3b82f6,stroke-width:2px,color:#fff
    style NetClient fill:#1a233a,stroke:#3b82f6,stroke-width:2px,color:#fff
    style DB_Eng fill:#0f172a,stroke:#10b981,stroke-width:2px,color:#fff
    style Main fill:#1e1b4b,stroke:#818cf8,stroke-width:1px,color:#fff
```
...
*   **Zarządzanie Połączeniami**: `socket.listen()` przyjmuje nowe połączenia, opcjonalnie zabezpiecza je warstwą SSL (jeśli `USE_SSL = True`) i deleguje do wątków.
*   **Threaded Architecture**:
    *   **Wątek Klienta**: Obsługuje pętlę `recv()`.
    *   **Async Turn Watchdog**: Wątek Turn Timer (`turn_timer_checker`) monitoruje czas bez blokowania globalnego stanu, co zapobiega zatorom przy wielu graczach.
*   **Matchmaking & Rooms**:
    *   Pokoje wykorzystują **Match Epoch** do synchronizacji stanu.
*   **Persistence Layer**: 
    *   Wykorzystanie `ThreadedConnectionPool` do zarządzania zasobami PostgreSQL.
    *   **Ranking Cache**: Top 100 wyników utrzymywane w RAM i odświeżane co 5 minut.
*   **Disconnects & Reconnects**: System nie obsługuje okna na powrót gracza (reconnect window). Po rozłączeniu jednego z graczy, pokój jest natychmiast usuwany z pamięci, a drugi gracz otrzymuje powiadomienie `opponent_disconnected` i wygrywa walkowerem, jeśli gra była w toku.
...
*   **Server Watchdog**: Funkcja `turn_timer_checker()` w `server.py` iteruje co sekundę po wszystkich pokojach. Jeśli gracz nie wykona akcji w ciągu 30 sekund (`current_time - room["last_action_time"] > 30`), serwer wysyła komunikat `turn_timeout`.
...
| `Niepoprawne dane.`                  | `VALIDATION_ERROR` | Hasło nie spełnia wymogów KAN-72 lub nick zawiera znaki niedozwolone. |
...
*   **Model Połączeń**: Serwer wykorzystuje **Connection Pooling** (`psycopg2.pool.ThreadedConnectionPool`). Zapobiega to nadmiernemu zużyciu zasobów PostgreSQL przez eliminację modelu "jeden proces na gracza". Połączenia są pobierane i zwracane do puli w cyklu transakcyjnym.
...
2.  **Instalacja zależności**:
    ```bash
    pip install pygame psycopg2-binary python-dotenv bcrypt pyinstaller
    ```
    *Uwaga systemowa (Linux/WSL2)*: Przy instalacji `psycopg2` ze źródeł wymagane mogą być systemowe biblioteki PostgreSQL (np. `sudo apt install libpq-dev python3-dev`). Zaleca się używanie `psycopg2-binary` dla środowisk deweloperskich.
3.  **Plik .env**: Stwórz plik `.env` w katalogu głównym na podstawie wzoru:
    ```text
    # Konfiguracja Bazy Danych (wymagana dla serwera)
    DB_HOST=localhost
    DB_NAME=battleships_db
    DB_USER=postgres
    DB_PASS=twoje_haslo

    # Konfiguracja Sieciowa (opcjonalna dla klienta i serwera)
...
2. **Klient**: `python main.py` (w celu połączenia z zewnętrznym serwerem należy ustawić zmienną `SERVER_HOST` w `.env` na IP hosta serwera).

### 6.4 Budowanie Wersji Wykonywalnych (.exe)
Projekt zawiera skrypty konfiguracyjne dla PyInstallera umożliwiające skompilowanie kodu źródłowego do autonomicznych plików wykonywalnych Windows (`dist/`):
1. **Serwer**: Uruchom `python build_server_exe.py` w celu wygenerowania `BattleShipsServer.exe` (tryb konsolowy do zbierania logów).
2. **Klient**: Uruchom `python build_exe.py` w celu wygenerowania `BattleShipsClient.exe` (tryb okienkowy bez konsoli w tle, z automatycznie wbudowanymi plikami graficznymi i dźwiękowymi).

### 6.5 Uruchomienie Wersji Wykonywalnych (Bez Środowiska Deweloperskiego)
Dzięki kompilacji za pomocą PyInstallera, dystrybucja gry w postaci plików `.exe` jest w pełni niezależna od środowiska lokalnego użytkownika:
*   **Interpreter Pythona i biblioteki**: Skompilowane pliki `.exe` posiadają wbudowany interpreter Pythona oraz wszystkie niezbędne biblioteki dynamiczne (`pygame`, `bcrypt`, `psycopg2`, itp.). **Nie jest wymagane instalowanie Pythona ani żadnych zależności (`pip`) na komputerze docelowym gracza lub serwera.**
*   **Zasoby statyczne (Klient)**: Obrazy, czcionki i pliki dźwiękowe zostały spakowane wewnątrz pliku `BattleShipsClient.exe` i są automatycznie wypakowywane do pamięci tymczasowej przy uruchomieniu.
*   **Wymagania wdrożeniowe**:
    *   **Serwer**: Wymaga pliku `.env` z danymi bazy PostgreSQL w tym samym folderze, a sama baza PostgreSQL musi być uruchomiona.
