...
def battle_phase(screen, p1_name, p2_name, net, player_idx, initial_turn, my_fleet):
    ...
    while True:
        ...
        screen.blit(turn_surf, turn_surf.get_rect(center=(WIDTH // 2, 80)))

        # Etykiety plansz
        my_label = font_ui.render("TWOJA FLOTA", True, TEXT_COLOR)
        screen.blit(my_label, my_label.get_rect(center=(my_grid_x + grid_size // 2, my_grid_y - 80)))

        enemy_label = font_ui.render("TARCZA WROGA", True, TEXT_COLOR)
        screen.blit(enemy_label, enemy_label.get_rect(center=(enemy_grid_x + grid_size // 2, enemy_grid_y - 80)))
        ...
        screen.blit(my_score_surf, my_score_surf.get_rect(center=(my_grid_x + grid_size // 2, my_grid_y - 30)))

        enemy_score_surf = font_score.render(f"Punkty: {enemy_score}", True, (255, 215, 0))
        screen.blit(enemy_score_surf,
                    enemy_score_surf.get_rect(center=(enemy_grid_x + grid_size // 2, enemy_grid_y - 30)))

        # Rysowanie plansz
        draw_grid(screen, my_grid_x, my_grid_y, grid_size)
        draw_grid(screen, enemy_grid_x, enemy_grid_y, grid_size)

        # -- NOWOŚĆ: Rysowanie własnej floty na lewej planszy --
        for ship in my_fleet:
            col, row = ship.grid_pos
            ship_w = ship.length * cell_size if ship.horizontal else cell_size
            ship_h = cell_size if ship.horizontal else ship.length * cell_size

            ship_rect = pygame.Rect(my_grid_x + col * cell_size, my_grid_y + row * cell_size, ship_w, ship_h)

            pygame.draw.rect(screen, ship.color, ship_rect, border_radius=8)
            pygame.draw.rect(screen, (255, 255, 255), ship_rect, 2, border_radius=8)

        # Rysowanie strzałów na Tarczy Wroga (Prawa)
        for x, y in my_shots_hit:
            pygame.draw.circle(screen, (255, 50, 50), (enemy_grid_x + x * cell_size + cell_size // 2,
                                                       enemy_grid_y + y * cell_size + cell_size // 2), cell_size // 3)
        for x, y in my_shots_miss:
            pygame.draw.circle(screen, (150, 150, 150), (enemy_grid_x + x * cell_size + cell_size // 2,
                                                         enemy_grid_y + y * cell_size + cell_size // 2), cell_size // 3)
